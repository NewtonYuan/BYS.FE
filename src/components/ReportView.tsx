...
export default function ReportView({ report }: { report: LeaseReport }) {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-neutral-800 bg-neutral-950/30 p-5">
          ...
          <div className="mt-4 space-y-3">
            {report.keyPoints.map((k, idx) => (
              <div key={idx} className="rounded-xl border border-neutral-800 bg-neutral-900 p-4">
                ...
                <p className="mt-2 text-sm text-neutral-300">{k.whyItMatters}</p>
              </div>
            ))}
          </div>
        ) : (
          <p className="mt-3 text-sm text-neutral-400">No watch points were detected from the extracted text.</p>
        )}
      </section>
      ...
      <section className="rounded-2xl border border-neutral-800 bg-neutral-950/30 p-5">
        ...
        {report.keyDates?.length ? (
          <div className="mt-3 divide-y divide-neutral-800 rounded-xl border border-neutral-800 bg-neutral-900">
            {report.keyDates.map((d, idx) => (
              <div key={idx} className="flex items-start justify-between gap-4 p-4">
                <div>
                  <div className="font-medium">{d.label}</div>
                  {d.notes ? <div className="mt-1 text-xs text-neutral-400">{d.notes}</div> : null}
                </div>
                <div className="text-sm text-neutral-200">{d.date ?? "—"}</div>
              </div>
            ))}
          </div>
        ) : (
          <p className="mt-3 text-sm text-neutral-400">No key dates were extracted.</p>
        )}
      </section>

      <div className="rounded-2xl border border-neutral-800 bg-neutral-950/30 p-4 text-xs text-neutral-400">
